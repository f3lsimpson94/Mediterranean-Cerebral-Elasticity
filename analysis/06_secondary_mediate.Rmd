...
```{r}
# 2) Load & prepare data -----------------------------------------------------
dat <- read.csv("C:/Users/c3371138/Dropbox/optical-imaging/AF_optical_3factor_c.csv") %>%
...
```{r}
# 7) Build mediation table (a, b, c′, ab, total; β_std, 95% perc CI, p) ------
...
# Build one 5-row block (a, b, Direct, Indirect, Total) for each outcome
build_block <- function(fit, outcome_label, out_var) {
  a_path   <- extract_param(fit, label = "a")
  b_path   <- extract_param(fit, label = "b")
  ...
  tibble::tibble(
    ...
    `Effect Type` = c(
    "a path (MedDiet → PReFx)",
    "b path (PReFx → outcome)",
    "c′ path (direct)",
    "a*b path (indirect)",
    "Total"
          ),
    β             = c(a_path$beta, b_path$beta, direct$beta, indirect$beta, total$beta),
    `95% CI`      = c(a_path$CI,   b_path$CI,   direct$CI,   indirect$CI,   total$CI),
    p             = c(a_path$p,    b_path$p,    direct$p,    indirect$p,    total$p)
  )
}
...
knitr::kable(
  med_table,
  caption = "Mediation analysis of PReFx as a mediator of the association between MedDiet adherence and cognitive outcomes (standardised β, bootstrap 95% CI)"
)
```
